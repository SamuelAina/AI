...
class PlanningGraph():
    ...
    def add_literal_level(self, level):
        ...
        for action in self.all_actions:
            pgNodeAction = PgNode_s(action)
            if pgNodeAction.prenodes.issubset(self.a_levels[level]):
                self.a_levels[level].add(pgNodeAction)
                for pgNodeState in pgNodeAction.prenodes:
                    pgNodeState.children.add(pgNodeAction)
                    pgNodeAction.parents.add(pgNodeState)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        #REVIEWER ADVICE
        #For your reference, here is another way that passes the tests:
